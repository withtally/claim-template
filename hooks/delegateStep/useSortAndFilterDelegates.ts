...
export const useSortAndFilterDelegates = ({ delegates }: IParams) => {
  ...
  // Sort States
  const [sortOptionValue, setSortOptionValue] = useState<SortOptionsEnum>(
    SortOptionsEnum.VotingPower,
  );
  // Pagination states
  const [amountOfDelegatesToShow, setAmountOfDelegatesToShow] =
    useState<number>(amountOfDelegatesPerPage);
  ...
  const loadNextChunkOfDelegates = useCallback(() => {
    setAmountOfDelegatesToShow(
      (prevAmount) => prevAmount + amountOfDelegatesPerPage,
    );
  }, []);
  ...
  const focusAreasOptions = useMemo(() => {
    return UIconfig.focusAreas.map((option) => ({
      value: option,
      text: option,
    }));
  }, []);
  ...
  const sortOptions = useMemo(() => {
    return Object.values(SortOptionsEnum).map((option) => ({
      value: option,
      text: `Sort by: ${option}`,
    }));
  }, []);
  ...
  const filteredDelegates = useMemo(() => {
    ...
    const filteredDelegatesArr = delegates.filter((delegate) => {
      ...
      if (debouncedSearchValue) {
        const addressMatches =
          delegate.account.address === debouncedSearchValue;
      ...
      if (selectedArea) {
        if (delegate?.statement?.issues) {
          const foundIssue = delegate.statement.issues.find(
            (issue) => issue.name === selectedArea,
          );
          areasMatches = !!foundIssue;
        } else {
          areasMatches = false;
        }
      }
    ...
    switch (sortOptionValue) {
      ...
      case SortOptionsEnum.ReceivedDelegations:
        sortedDelegatesArr = filteredDelegatesArr.sort(
          (delegate1, delegate2) => {
  ...
  const paginatedDelegates = useMemo(() => {
    ...
    paginatedDelegatesArr.length =
      amountOfDelegatesToShow > filteredDelegates.length
